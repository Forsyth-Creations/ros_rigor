...
def generate_launch_description():
    ...
    config_file_launch_config = LaunchConfiguration('config_file', default='/Robots/hermes_robot/config/topic_mapping.yaml')
    ...
    ros_gz_bridge = Node(
        ...
        output='screen',
        parameters=[{"config_file": "/Robots/hermes_robot/config/topic_mapping.yaml"}]
    )

    # Define robot description for Robot State Publisher
    pkg_project_description = get_package_share_directory('hermes_robot')
    sdf_file = os.path.join(pkg_project_description, 'urdf', 'hermes_robot_2.urdf')
    ...
    launch_arguments={'model': '/Robots/hermes_robot/urdf/hermes_robot.urdf'}.items(),
    )
