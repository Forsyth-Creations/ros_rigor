...
class Module(Node):
    def __init__(self, name):
        ...
        self.module_name = (
            self.declare_parameter("module_name", "module_default")
            .get_parameter_value()
            .string_value
        )
        self.invert_drive_for_dime = (
            self.declare_parameter("invert_drive_for_dime", False)
            .get_parameter_value()
            .bool_value
        )
        self.dime_angle = (
            self.declare_parameter("dime_angle", 0.785)
            .get_parameter_value()
            .double_value
        )  # The angle to go to when on a dime
        self.mock_encoder_values = (
            self.declare_parameter("mock_encoder_values", True)
            .get_parameter_value()
            .bool_value
        )  # For running without actual hardware
        self.invert_drive_motor = (
            self.declare_parameter("invert_drive_motor", False)
            .get_parameter_value()
            .bool_value
        )  # For running without actual hardware
        # self.invert_drive_motor = True
        self.get_logger().info(
            f"Creating Module: {name} with dime angle : {self.dime_angle}"
        )

        # Print out that the invert_drive_motor parameter is set
        if self.invert_drive_motor:
            self.get_logger().info(
                f"{Fore.GREEN}Inverting Drive Motor for {self.module_name}{Fore.RESET}"
            )

        # Publish Encoder values and limit switch state
        self.encoder_sub = self.create_publisher(
            JointState, f"/{self.module_name}/encoders", 10
        )
        self.limit_switch_sub = self.create_publisher(
            Float64, f"/{self.module_name}/limit_switch", 10
        )

        # Pub/Sub for target pivot angle
        self.rqst_pivot_sub = self.create_subscription(
            Float64,
            f"/{self.module_name}/rqst_pivot_direction",
            self.set_rqst_pivot_direction,
            10,
        )
        self.pivot_position_pub = self.create_publisher(
            Float64, f"/{self.module_name}/pivot_position", 10
        )

        # Pub/Sub for motor speed
        self.drive_motor_sub = self.create_subscription(
            Float64,
            f"/{self.module_name}/rqst_wheel_speed",
            self.set_rqst_wheel_speed,
            10,
        )
        self.drive_motor_pub = self.create_publisher(
            Float64, f"/{self.module_name}/wheel_speed", 10
        )
        ...
        self.drive_pid_controller = PIDController(
            p=0.1,
            i=0.4,
            d=0.01,
            target=self.rqst_wheel_speed,
            max_output=2,
            min_output=-2,
        ...
        self.pivot_pid_controller = PIDController(
            p=0.1,
            i=0.1,
            d=0.01,
            target=self.rqst_pivot_angle,
            max_output=6.28,
            min_output=0.0,
            logger=self.get_logger(),
        )
        ...
        self.mode_sub = self.create_subscription(
            Int8, f"/{self.module_name}/rqst_mode", self.set_mode, 10
        )
    ...
    def set_rqst_pivot_direction(self, angle):
        ...
        if self.mode == 1:
            ...
            self.get_logger().warn(
                f"{Fore.YELLOW}{self.module_name}: Direct angle request ignored in Dime Mode{Fore.RESET}"
            )
            self.rqst_pivot_angle = self.dime_angle
        else:
    ...
    def update(self):
        ...
        # -------------------------- Driving Wheel --------------------------------------
        drive_output = self.drive_pid_controller.compute_move(
            self.rqst_wheel_speed, self.actual_wheel_speed
        )

        if self.invert_drive_for_dime and self.mode == 1:
            self.drive_wheel_position -= (
                drive_output if not self.invert_drive_motor else -drive_output
            )
        else:
            self.drive_wheel_position += (
                drive_output if not self.invert_drive_motor else -drive_output
        ...
        pivot_output = self.pivot_pid_controller.compute_move(
            modified_rqst_pivot_angle, self.actual_pivot_position
        )
        pivot_msg = Float64()
